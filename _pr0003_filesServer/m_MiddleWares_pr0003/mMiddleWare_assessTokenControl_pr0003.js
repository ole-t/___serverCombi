...
export default function mMiddleWare_assessTokenControl_pr0003(req, res, next) {

    // console.log("");
    // console.log("============= ");
    // console.log("ЗАПУСК mMiddleWare_assessTokenControl_pr0003, req.url= " + req.url);
    ...
    if ((req.method == "POST")
        ...
        if (!req.headers.accesstoken) {
            ...
            res.status(401).json("pr0003 --- m User is not auth");
        ...
        // console.log("=== ");
        let decodeValidationToken = true;
        // Исправить - Активировать
        // decodeValidationToken = varsANDfunctions_fromPostService_pr0003.validateAccessToken(req.headers.accesstoken);
        ...
        let finedUserIndex = varsANDfunctions_fromPostService_pr0003.findUser_Index_inReestr(decodeValidationToken.user_Email, decodeValidationToken.mKuiir);
        if (!(finedUserIndex >= 0)) {
        ...
        if (
            ...
            !vars_and_functions___pr0003.users_Reestr[finedUserIndex].autorisationData.tokensDifferentGadgets[decodeValidationToken.gadget_process_ID]
        ) {
        ...
        if (
            ...
            vars_and_functions___pr0003.users_Reestr[finedUserIndex].autorisationData.tokensDifferentGadgets[decodeValidationToken.gadget_process_ID].accessToken != req.headers.accesstoken
        ) {
        ...
        if (presenceOfProgramEerrors == false) {
            ...
            req.headers.decodeAT_____mGadgetProcess_ID = decodeValidationToken.gadget_process_ID;
        }
    }

    // Если проверка пройдена - продолжаем обработку сервером
    if (presenceOfProgramEerrors == false) {
        // console.log("Проверка выполнена успешно, продолжаем выполнение next()");
        next();
    }
    else {
