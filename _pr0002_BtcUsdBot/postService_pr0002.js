...
async function proc___postService_pr0002() {
    ...
    // Активировать
    let connection_to_telegramBot___BTC_USD_signal_pr0002 = "waitinConnection";
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                binanceSocket.onclose = () => {
                    ...
                    binanceSocket = null; // Проверить, нужно ли это. Тут сбрасываем ссылку, чтобы знать, что соединения больше нет. Фактически, чтобы очистить память от старых подключений            
                    setTimeout(() => {
                        binanceConnection();
                ...
                case 'newUserRegistration': {
                    try {
                        ...
                        await global_Functions_and_Servises_forAll_Projects.telegramBot_Servise.messegeToCurrentTelegramBot(
                            ...
                            varrsAndData_pr0002.emodziListTelegram_pr0002.default_currentProjectEmodzi //емодзи из переменной, из списка
                        )

                        // отправляем приветствие новому пользователю
                        global_Functions_and_Servises_forAll_Projects.telegramBot_Servise.messegeToCurrentTelegramBot(
    ...
    async function checkSingleAlerts_forAll_Users() {
    ...
    async function checkRepeatAlerts_forAll_Users() {
        while (true) { // это бесконечный цикл, выполняет свой блок кода постоянно, пока не произойдёт явный выход из него (break, return, throw и т.п.).  
            // 🔁 Ждём 10 секунд перед следующей итерацией
...
export async function sendTelegramInfo_from_pr0002(text, additional__emodzi_or_name_or_color_emodzi) {
    try {
        ...
        return await global_Functions_and_Servises_forAll_Projects.telegramBot_Servise.messegeToCurrentTelegramBot(
            ...
            (config_pr0002.emodziListTelegram_currentProject.default_currentProjectEmodzi + secondEmodzi + " ") //емодзи из переменной, из списка 
        )
